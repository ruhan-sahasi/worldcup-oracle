...
| **Correlated tournament shocks** (`oracle-sim::ShockModel`) - the per-iteration team strength shocks were **independent** across teams and across attack/defence; a two-parameter factor model (2x2 Cholesky within a team, plus an antisymmetric tournament-wide scoring environment) makes that assumption variable, with each team's **marginal variance preserved** so correlation cannot silently widen the model's confidence | an assumption that moved the title picture more than five of the model's named signals was previously invisible - no config expressed it, no test varied it, no output hinted at it. Defaults to exact independence, so no published forecast changes; `--shock-attack-defence` / `--shock-environment` and a sensitivity row quantify what it is worth |
...
| **Predicted bracket** - the single most likely completion of the knockout bracket, every remaining tie resolved to its favourite up to a projected champion, with the joint probability of that exact bracket | a concrete "here is what the model expects", plus an honest number for how unlikely any exact bracket is |
| **Match leverage** - each current-round tie scored by the total swing in champion odds between its two results (re-running the bracket DP under each) | which game matters most to the title race, accounting for who sits where in the bracket, not just how close the tie is |
...
| **Knockout factors** (per-team **penalty-shootout skill** + **knockout pedigree**, plus **extra-time fatigue** carry-over) | knockout-only effects open-play strength can't capture: who wins shootouts, who handles single-elimination pressure (debutant-heavy in a 48-team field), and a side drained by extra time the round before |
...
# Ask what the simulator's independence assumption is worth (default 0 = independent).
cargo run --release -p oracle-cli -- simulate --shock-attack-defence 0.5
